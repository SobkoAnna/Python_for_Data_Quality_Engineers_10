...
class Main:
    ...
    class RentInfo:
        ...
        def find_apartments(self):
            if self.status == "No data available for this city":
                return 0
            return  random.randint(1,20)

        def create_news(self):
            self.rent_query = f"City : {self.city}, Budget: {self.budget}"
            self.notes = f"Status: {self.status}, Apartments found: {self.apartments_found}"
        ...
        while True:
            ...
            if choice == "3":
                ...
                if city in list_city:
                    budget = int(input("Enter your rental budget per month: "))
                    self.RentInfo(menu, city, budget)
                    print()
                else:
