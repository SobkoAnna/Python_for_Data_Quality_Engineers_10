...
#        def __init__(self, cur, sql_ntext=None, sql_ncity=None, sql_atext=None, sql_expiration_date=None, sql_rcity=None, sql_rent_description=None, sql_rent_price=None):
        def __init__(self, sql_ntext=None, sql_ncity=None, sql_atext=None, sql_expiration_date=None,
                         sql_rcity=None, sql_rent_description=None, sql_rent_price=None):

            self.sql_ntext = sql_ntext
            self.sql_ncity = sql_ncity
            self.sql_atext = sql_atext
            self.sql_expiration_date = sql_expiration_date
            self.sql_rcity = sql_rcity
            self.sql_rent_description = sql_rent_description
            self.sql_rent_price = sql_rent_price
        ...
        def t_exist(self, table_name):
            self.cur.execute(f"SELECT name FROM sqlite_master WHERE type='table' AND name='{table_name}'")
            result = self.cur.fetchone()
            return result is not None

        def record_exists(self, table_name, colum1, colum2, colum1_value, colum2_value):
            self.cur.execute(f'SELECT COUNT(*) FROM {table_name} WHERE {colum1}="{colum1_value}" AND {colum2}="{colum2_value}"')
            count = self.cur.fetchone()[0]
            return count > 0

        def SqlNews(self, sql_ntext=None, sql_ncity=None):
            table_name = "news"
            self.sql_ntext = sql_ntext
            self.sql_ncity = sql_ncity
            if not self.t_exist(table_name):
                self.cur.execute(f"CREATE TABLE {table_name} (text, city)")
                self.cur.commit()
            if self.record_exists(table_name, "text", "city", self.sql_ntext, self.sql_ncity):
                print("Duplicate record")
                self.cur.commit()
            else:
                self.cur.execute(f"INSERT INTO {table_name} VALUES('{self.sql_ntext}', '{self.sql_ncity}')")
                self.cur.commit()
            self.cur.execute(f'SELECT * FROM {table_name}')
            self.cur.commit()
            return self.cur.fetchall()

        def SqlAD(self, sql_atext=None, sql_expiration_date=None):
            table_name = 'private_ad'
            self.sql_atext = sql_atext
            self.sql_expiration_date = sql_expiration_date
            if not self.t_exist(table_name):
                self.cur.execute(f"CREATE TABLE {table_name} (text, date)")
                self.cur.commit()
            if self.record_exists(table_name, 'text', 'date', self.sql_atext, self.sql_expiration_date):
                print("Duplicate record")
                self.cur.commit()
            else:
                self.cur.execute(f"INSERT INTO {table_name} VALUES('{self.sql_atext}', '{self.sql_expiration_date}')")
                self.cur.commit()
            self.cur.execute(f'SELECT * FROM {table_name}')
            self.cur.commit()
            return self.cur.fetchall()

        def SqlRentInfo(self, sql_rcity=None, sql_rent_description=None, sql_rent_price=None):
            table_name = 'rentInfo'
            self.sql_rcity = sql_rcity
            self.sql_rent_description = sql_rent_description
            self.sql_rent_price = sql_rent_price
            if not self.t_exist(table_name):
                self.cur.execute(f"CREATE TABLE {table_name} (city,description,price)")
                self.cur.commit()
            if self.record_exists(table_name, 'city', 'description', self.sql_rcity, self.sql_rent_description):
                print("Duplicate record")
                self.cur.commit()
            else:
                self.cur.execute(f"INSERT INTO {table_name} VALUES('{self.sql_rcity}', '{self.sql_rent_description}', '{self.sql_rent_price}')")
                self.cur.commit()
            self.cur.execute(f'SELECT * FROM {table_name}')
            self.cur.commit()
            return self.cur.fetchall()
    ...
    def run(self):
        while True:
            ...
            if choice == "7":
                dataapi = self.DatabaseApi()
                print('1. News  2. Private ad   3. Rent Info')
                choice = input("Select type of data by number of the category: ")
                if choice == "1":
                    sql_ntext = input("News text: ")
                    sql_ncity = input("City: ")
                    self.DatabaseApi.SqlNews(dataapi, sql_ntext, sql_ncity)
                    print()
                if choice == "2":
                    sql_atext = input("Private AD text: ")
                    sql_expiration_date = input("Please input expiration date :")
                    self.DatabaseApi.SqlAD(dataapi, sql_atext, sql_expiration_date)
                    print()

                if choice == "3":
                    sql_rcity = input("Enter your city: ")
                    sql_rent_description = input("Enter description: ")
                    sql_rent_price = int(input("Enter your rental price per month: "))
                    self.DatabaseApi.SqlRentInfo(dataapi, sql_rcity, sql_rent_description, sql_rent_price)
                    print()
